...
class AnimesamaProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // Sécurité pour éviter le crash si pas de données
        if (rawSeasonData.isEmpty()) return newAnimeLoadResponse(title, url, TvType.Anime)

        val extractedData = rawSeasonData.split("/*", "*/").getOrElse(2) { "" }
        ...
        panneauMap.forEach { (seasonName, alias) ->
            ...
            if (urlTransforme.isNotEmpty() && urlTransforme.last() != "vf") {
    ...
    private suspend fun retreiveSrcs(streamPage: String): Map<String, List<String>> {
        ...
        if (request.isSuccessful) {
            ...
            val scriptElem = doc.selectFirst("#sousBlocMiddle script") ?: return mapOf()
            val epiKey = scriptElem.toString()
            val re = Regex("""<script[^>]*src=['"]([^'"]*episodes\.js\?filever=\d+)['"][^>]*>""")
            val episodeKey = re.find(epiKey)?.groupValues?.get(1) ?: return mapOf()
            
            val rawLinksRequest = app.get("$streamPage/$episodeKey")
            if (!rawLinksRequest.isSuccessful) return mapOf()
            
            val rawLinks = rawLinksRequest.text
